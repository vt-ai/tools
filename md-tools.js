// md-tools.js — logic for the two Markdown tools.

import { downloadBlob, wireDropzone, wireToolNav } from './shared.js';

import { marked } from 'https://cdn.jsdelivr.net/npm/marked@12/+esm';
import { Document, Packer, Paragraph, TextRun, HeadingLevel } from 'https://cdn.jsdelivr.net/npm/docx@8/+esm';
...
// =================================================================
// TO MARKDOWN (Word / Excel; PDF handled by the LiteDoc iframe above)
// =================================================================
(function toMarkdownTool() {
  const dz = document.getElementById('dz-tomd'), fi = document.getElementById('fi-tomd');
  const outputWrap = document.getElementById('tomd-output');
  const preview = document.getElementById('tomd-preview');
  ...
  wireDropzone(dz, fi, async (file) => {
    ...
    try {
      ...
      if (name.endsWith('.docx')) {
        ...
        currentMd = td.turndown(result.value);
        ...
        setStatus('tomd-status', 'unsupported format — use .docx, .xlsx, .xls, or .csv (PDFs: use the LiteDoc box above)');
        return;
      }
      ...
      console.error(e);
      setStatus('tomd-status', 'error: ' + e.message);
    }
  });

  document.getElementById('tomd-download').onclick = () => {
    if (!currentMd) return;
    downloadBlob(new Blob([currentMd], { type: 'text/markdown' }), outName || 'output.md');
  };
})();
...
(function fromMarkdownTool() {
  const input = document.getElementById('frommd-input');
  const preview = document.getElementById('frommd-preview');
    ...
    try {
      ...
      for (const token of tokens) {
        if (token.type === 'heading') {
          ...
          children.push(new Paragraph({ heading: level, children: parseInline(token.tokens) }));
        } else if (token.type === 'paragraph') {
          children.push(new Paragraph({ children: parseInline(token.tokens) }));
        } else if (token.type === 'list') {
          token.items.forEach((item, i) => {
            const prefix = token.ordered ? `${i + 1}. ` : '• ';
            const runs = [new TextRun({ text: prefix })];
            if (item.tokens) item.tokens.forEach(st => {
              if (st.type === 'text') runs.push(new TextRun({ text: st.text }));
              else if (st.type === 'paragraph') runs.push(...parseInline(st.tokens));
            });
      ...
      console.error(e);
      setStatus('frommd-status', 'error generating DOCX: ' + e.message);
    }
  };

  document.getElementById('frommd-pdf').onclick = async () => {
    setStatus('frommd-status', 'rendering PDF… (may take a second)');
    try {
      ...
      style.textContent = `h1{font-size:24pt;margin-bottom:8pt;font-weight:bold;}h2{font-size:18pt;margin-bottom:6pt;font-weight:bold;}h3{font-size:14pt;margin-bottom:4pt;font-weight:bold;}p{margin-bottom:8pt;}ul,ol{margin-left:20pt;margin-bottom:8pt;}pre{background:#f4f4f4;padding:10pt;border-radius:4pt;overflow-x:auto;font-size:10pt;}code{font-family:'Courier New',monospace;}blockquote{border-left:3px solid #ccc;padding-left:10pt;color:#555;}`;
      ...
      console.error(e);
      setStatus('frommd-status', 'error generating PDF: ' + e.message);
    }
  };
})();
